...
BOOL CMenuWnd::Receive(ContextMenuParam param)
{
	...
	case 3:
		{
			// 设置
			//::PostMessage(GetParent(m_hWnd), UWM_SHOW_SET_DLG, 2, 0);
			Close();
		}
		break;
	case 6:
		{
			// 退出按钮
			//::PostMessage(GetParent(m_hWnd), UWM_SHOW_SET_DLG, 4, 0);
			Close();
		}
		break;
	case 4:
		{
			// 关于
			//::PostMessage(GetParent(m_hWnd), UWM_SHOW_SET_DLG, 3, 0);
			Close();
		}
		break;
	default:
...
LRESULT CMenuWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if( uMsg == WM_CREATE ) 
	{
		if( m_pOwner != NULL) 
		{
			...
			//m_pLayout->SetBkColor(0xff11191d);
			//m_pLayout->SetBorderColor(0xff11191d);
	...
	else if(uMsg == WM_LBUTTONDOWN)
	{
		...
		DuiLib::CControlUI* pNowCtrl = m_pm.FindControl(pt);
		if (pNowCtrl!=NULL)
		{
			if(_T("btn_mainpage_bj0") == pNowCtrl->GetUserData())
			{
				//::PostMessage(GetParent(GetParent(m_hWnd)), UWM_SHOW_SET_DLG, 5, 0);
				Close();
			}
			else if(_T("btn_mainpage_bj1") == pNowCtrl->GetUserData())
			{
				//::PostMessage(GetParent(GetParent(m_hWnd)), UWM_SHOW_SET_DLG, 6, 0);
				Close();
			}
		}
	}
